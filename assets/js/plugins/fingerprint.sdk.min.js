...
(function (Fingerprint) {
  ...
  (function (DeviceUidType) {
    ...
    (DeviceUidType =
      Fingerprint.DeviceUidType || (Fingerprint.DeviceUidType = {}))
  );
  var DeviceModality;
  (function (DeviceModality) {
    ...
    (DeviceModality =
      Fingerprint.DeviceModality || (Fingerprint.DeviceModality = {}))
  );
  ...
  (function (DeviceTechnology) {
    ...
    (DeviceTechnology =
      Fingerprint.DeviceTechnology || (Fingerprint.DeviceTechnology = {}))
  );
  var SampleFormat;
    ...
    WebApi.prototype.getDeviceInfo = function (deviceUid) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var command = new Command(
          ...
          strToB64Url(JSON.stringify(deviceParams))
        );
        var request = new Request(command, resolve, reject);
        _instance.requests.push(request);
        if (_instance.webChannel.isConnected()) _instance.processQueue();
    ...
    WebApi.prototype.startAcquisition = function (sampleFormat, deviceUid) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var command = new Command(
          ...
          strToB64Url(JSON.stringify(acquisitionParams))
        );
        var request = new Request(command, resolve, reject);
        _instance.requests.push(request);
        if (_instance.webChannel.isConnected()) _instance.processQueue();
    ...
    WebApi.prototype.processQueue = function () {
      for (var i = 0; i < this.requests.length; i++) {
        ...
        this.webChannel.sendDataTxt(
          strToB64Url(JSON.stringify(this.requests[i].command))
        );
      ...
      if (request) {
        if (response.Method === Method.EnumerateDevices) {
          if (response.Result < 0 || response.Result > 2147483647)
            request.reject(
              new Error(
                "EnumerateDevices: " + (response.Result >>> 0).toString(16)
              )
            );
          else {
            var enumerateDevicesResponse = JSON.parse(
              b64UrlToUtf8(response.Data)
            );
          ...
          if (response.Result < 0 || response.Result > 2147483647)
            request.reject(
              new Error(
                "GetDeviceInfo: " + (response.Result >>> 0).toString(16)
              )
            );
          else {
          ...
          if (response.Result < 0 || response.Result > 2147483647)
            request.reject(
              new Error(
                "StartAcquisition: " + (response.Result >>> 0).toString(16)
              )
            );
          ...
          if (response.Result < 0 || response.Result > 2147483647)
            request.reject(
              new Error(
                "StopAcquisition: " + (response.Result >>> 0).toString(16)
              )
            );
          else request.resolve();
        }
